...
def mask_account_card(card: str) -> str:
    ...
    for i in range(len(card), 0, -1):
        if card[i - 1].isdigit():
            ...
            break
    if "Счет" in card:
        if masks.get_mask_account(card[index_of_the_card: len(card)]) == "Недопустимый номер счета":
            return "Недопустимый номер счета"
        else:
            mask_number = "Счет " + masks.get_mask_account(card[index_of_the_card: len(card)])
    else:
        if masks.get_mask_card_number(card[index_of_the_card: len(card)+1]) == "Недопустимый номер карты":
